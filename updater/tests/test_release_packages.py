...
def test_discovery_rejects_unexpected_windows_package(tmp_path: Path):
    ...
    assert {package.filename for package in packages} == {f"conda-runtime-{version}-0.conda"}
    windows = tmp_path / "win-64"
...
def test_windows_remote_checks_use_conda_runtime(
    ...
    assert release_packages.api_has(package, "jezdez")
    assert release_packages.repodata_has(package, "jezdez")
    assert any("/package/jezdez/conda-runtime" in url for url in urls)
